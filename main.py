...
def npp_scheduler(n, arrival, burst, priority, process_names):
    # Combine process details into a list of lists
    T = [[process_names[i], arrival[i], burst[i], priority[i], 0, 0, 0] for i in range(n)]
    
    # Sort processes by arrival time (from low to high)
    ...
    while completed < n:
        # Add processes to the ready queue if they have arrived and not yet completed
        for i in range(n):
            if T[i][1] <= current_time and T[i][2] > 0 and T[i] not in ready_queue:
                ready_queue.append(T[i])

        if ready_queue:
            # Sort ready queue by priority (lower number means higher priority)
            ready_queue.sort(key=lambda x: x[3])
            idx = ready_queue.pop(0)

            # Update Gantt chart
            if gantt_chart and gantt_chart[-1][0] == '-':
                # Fill in the previous idle segment up to the current time
            ...
            current_time += idx[2]
            idx[2] = 0  # Mark process as completed
            ...
            # If no process is ready, increment time and add idle time to Gantt chart
            if gantt_chart and gantt_chart[-1][0] == '-':
                gantt_chart[-1] = ('-', gantt_chart[-1][1], gantt_chart[-1][2] + 1)
            else:
                gantt_chart.append(('-', current_time, current_time + 1))
